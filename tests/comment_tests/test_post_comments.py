...
class TestPostComments(TestFlask):
    ...
    def test_make_post_comment(self):
        short_result = self.client.post(f'/post/{self.post.id}/comment', "foo")
        self.assert400(short_result)

        long_result = self.client.post(f'/post/{self.post.id}/comment', "a"*141)
        self.assert400(long_result)

        result = self.client.post(f'/post/{self.post.id}/comment', "foobarbazblargh")
        ...
        self.assertEqual(comment_result.json['rendered_text'], "foobarbazblargh")